...
async function searchRecipes(query) {
    try {
        ...
        // Выводим полный ответ от API в консоль для проверки
        console.log("Полный ответ от API:", data);

        // Проверка данных, чтобы убедиться, что results — это массив
...
async function showRecipeDetails(recipeId) {
    try {
        ...
        // Заполняем содержимое модального окна с данными рецепта
...
window.onload = () => {
    localStorage.removeItem("favorites"); // Удаляем избранные рецепты при загрузке страницы
    displayFavorites(); // Отображаем пустое избранное
};
